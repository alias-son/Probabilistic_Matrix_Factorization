...
movies = pd.read_csv('C:/RecoSys/Data/movies_metadata.csv', low_memory=False)
...
def content_recommender(title, cosine_sim=cosine_sim, movies=movies, indices=indices):
    ...
    sim_scores = sorted(sim_scores, key=lambda x: x[1], reverse=True)  # 1인 이유는 일련번호 말고 값을 뽑기위해
...
content_recommender('Batman Returns')
content_recommender('Born Free')
