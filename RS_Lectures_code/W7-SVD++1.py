...
class SVD_PP():
    ...
    def __init__(self, rating_matrix, K, alpha, beta, lamda, iterations, verbose=True):
        self.R = rating_matrix
        self.num_users, self.num_items = np.shape(self.R)
        self.K = K
        self.alpha = alpha
        self.beta = beta
        self.lamda = lamda   # 이것만 추가
...
SVD = SVD_PP(R_temp, K=90, alpha=0.001, beta=0.01, lamda=0.0001, iterations=300, verbose=True)
